...
export async function registerRoutes(
  ...
  app.post("/api/customers/import-balances", async (req, res) => {
    try {
      ...
      // Dynamic import to avoid issues if not used elsewhere, or just standard import if top-level
      ...
      console.log("Importing File. Rows found:", data.length);
      const debugInfo: any = {
      ...
      if (data.length > 0) {
        console.log("First row columns:", Object.keys(data[0] as object));
      }
      ...
      for (const row of data as any[]) {
        ...
        let customer = (await storage.getCustomers()).find(c => c.name.toLowerCase() === name.toString().toLowerCase());
        ...
        if (!customer) {
          try {
            ...
            results.errors++;
            results.details.push(`Failed to create customer: ${name}`);
            continue;
          }
        }
        ...
        try {
          ...
          results.errors++;
          results.details.push(`Failed to payment for: ${name}`);
        }
      }
  ...
  app.delete("/api/customer-payments/:id", async (req, res) => {
    try {
      const deleted = await storage.deleteCustomerPayment(req.params.id, req.user?.id);
      if (!deleted) {
        return res.status(404).json({ error: "Payment not found" });
      }
  ...
  app.delete("/api/vendor-payments/:id", async (req, res) => {
    try {
      const deleted = await storage.deleteVendorPayment(req.params.id, req.user?.id);
      if (!deleted) {
        return res.status(404).json({ error: "Payment not found" });
      }
      res.json({ success: true });
  ...
  app.delete("/api/hamali-cash/:id", async (req, res) => {
    try {
      const deleted = await storage.deleteHamaliCashPayment(req.params.id, req.user?.id);
      if (!deleted) {
  ...
  app.get("/api/reports/vendor-balances", async (req, res) => {
    try {
      const balances = await storage.getAllVendorBalances();
      res.json(balances);
    } catch (error) {
      console.error("Error fetching vendor balances:", error); // Added error logging
  ...
  app.get("/api/reports/customer-balances", async (req, res) => {
    try {
      ...
      console.error("Error fetching customer balances:", error); // Added error logging
