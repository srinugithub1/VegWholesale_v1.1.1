...
try {
    ...
    try {
        db.prepare(`ALTER TABLE invoices ADD COLUMN created_at INTEGER DEFAULT ${now}`).run();
    ...
    try {
        db.prepare(`ALTER TABLE invoices ADD COLUMN updated_at INTEGER DEFAULT ${now}`).run();
    ...
    try {
        db.prepare(`ALTER TABLE invoice_items ADD COLUMN created_at INTEGER DEFAULT ${now}`).run();
    ...
    try {
        db.prepare(`ALTER TABLE invoice_items ADD COLUMN updated_at INTEGER DEFAULT ${now}`).run();
