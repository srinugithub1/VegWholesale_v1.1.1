...
import { Loader2, Search, Filter, Edit, Save, ArrowUpDown, ArrowLeft, ArrowRight, Check, ChevronsUpDown, X, ShoppingBag, Calendar, Printer, Share2, Trash2 } from "lucide-react";
...
export default function CustomerEdit() {
    ...
    const updateItemMutation = useMutation({
        mutationFn: async (values: {
            id: string,
            invoiceId: string, // invoiceId is also uuid string
            weight: number,
            weight: number,
            price: number,
            bags: number,
    ...
    const handleDeleteClick = (id: string) => {
        if (confirm("Are you sure you want to delete this record? This will update the invoice total.")) {
                                ...
                                paginatedData.map((row, index) => (
                                    <TableRow key={row.itemId}>
                                        ...
                                        <TableCell>
                                            ...
                                            <Button variant="ghost" size="icon" onClick={() => handleDeleteClick(row.itemId)} className="text-red-500 hover:text-red-600 hover:bg-red-50">
