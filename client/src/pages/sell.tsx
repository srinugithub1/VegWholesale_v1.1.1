...
function VehicleSalePane({
  ...
  const handleCustomerChange = (value: string) => {
    if (value === "new") {
      onUpdateDraft({ ...draft, selectedCustomerId: "new", customerName: "", customerPhone: "", isCashSale: false });
    } else {
      ...
      onUpdateDraft({
        ...
        isCashSale: customer?.name.toLowerCase().includes("cash") || false
      });
    }
  };
  ...
  const createSaleMutation = useMutation({
    ...
    mutationFn: async () => {
      ...
      // const isCashCustomer = currentCustomerName.toLowerCase().includes("cash"); // REMOVED: Name-based logic
      const invoiceStatus = draft.isCashSale ? "completed" : "pending";
            ...
            {!draft.customerName && !draft.selectedCustomerId && (
              <>
                  ...
                  <PopoverTrigger asChild>
                    <Button
                      ...
                      className={cn(
                        ...
                        !draft.selectedCustomerId && "text-muted-foreground",
                        errors.customer ? "border-destructive ring-1 ring-destructive" : ""
                      )}
                    >
                      {draft.selectedCustomerId
                        ? customers.find((c) => c.id === draft.selectedCustomerId)?.name
                        : "Select Customer"}
                ...
                <Button
                  variant="outline"
                  size="icon"
                  className="h-8 w-8 shrink-0 bg-primary/10 hover:bg-primary/20 border-primary/20 text-primary"
                  onClick={() => {
                    onUpdateDraft({ ...draft, selectedCustomerId: "new", customerName: "", customerPhone: "", isCashSale: false });
                  }}
            ...
            {(draft.customerName || draft.selectedCustomerId === "new") && (
              <div className="flex-1 flex items-center gap-2">
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    onUpdateDraft({
                      ...draft,
                      customerName: name,
                      isCashSale: name.toLowerCase().includes("cash")
                    });
                ...
                <Button
                  ...
                  size="icon"
                  className="h-8 w-8"
                  onClick={() => onUpdateDraft({ ...draft, selectedCustomerId: "", customerName: "", customerPhone: "", isCashSale: false })}
                >
                  <X className="h-4 w-4" />
                </Button>
              </div>
